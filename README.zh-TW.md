...
### 全域函式庫注入 (Global Library Injection)

你可以將常用的函式庫（如 `lodash`、`date-fns`）或自定義工具函式一次性注入到所有的模板運算式中。這些變數可以直接在 `{{ }}` 中存取。
...
> [!TIP]
> `createComponentRegistry` 同樣接受 `globals` 作為第二個參數，確保預先解析的工廠也能存取這些工具。
